import {recognizeImage,RECOGNITION_VERSION} from './recognition.js?v=b1cae4b4be94';
import {validPolygon,polygonBounds,insideRoom,roomAnchor} from './geometry.js?v=b1cae4b4be94';
import {regionWithin} from './intake-validation.js?v=b1cae4b4be94';
import {ROOMS,uid} from './core.js?v=b1cae4b4be94';
...
export async function editRecognition(f,{modal:openModal,toast,save,manual,replace}){
 const $=s=>document.querySelector(s),all=s=>[...document.querySelectorAll(s)];
 const modal=(title,body,buttons)=>openModal(title,body,buttons,{className:'recognition-modal'});
 ...
 if(!r.candidates.length&&!r.boundary){
  modal('这张图未能自动分区',`<div class="recognition-empty" data-recognition-status="empty"><img src="${e(f.image)}" alt="当前户型图"><p>没有识别到可采用的房间或外轮廓。</p><p>自动识图目前适合黑白二维线稿。彩色效果图请保留底图，直接勾画。</p><small>已画好的红线、房间和家具都保留。</small>${replace?'<button class="text-link" id="recognition-replace">返回上传页 →</button>':''}</div>`,`<button class="btn ghost" data-close>返回</button>${manual?'<button class="btn primary" id="recognition-manual">在原图上勾画 →</button>':''}`);
  $('#recognition-manual')?.addEventListener('click',()=>{$('#modal').close();manual();});
  $('#recognition-replace')?.addEventListener('click',()=>{$('#modal').close();replace();});
  return;
 }
