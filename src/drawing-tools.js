import {insideRoom} from './geometry.js?v=11c9ab943438';
...
export function bindViewport(root,key,{cancel=()=>{}}={}){
 ...
 let fitW=0,fitH=0;
 const apply=()=>{const w=root.clientWidth,h=root.clientHeight,aspect=Number(root.dataset.aspect)||w/h;fitW=Math.min(w,h*aspect);fitH=fitW/aspect;sheet.style.width=fitW+'px';sheet.style.height=fitH+'px';const sw=fitW*state.scale,sh=fitH*state.scale;state.x=sw<=w?(w-sw)/2:clamp(state.x,w-sw,0);state.y=sh<=h?(h-sh)/2:clamp(state.y,h-sh,0);sheet.style.transformOrigin='0 0';sheet.style.transform=`translate(${state.x}px,${state.y}px) scale(${state.scale})`;sheet.dataset.zoom=state.scale;root.dataset.zoom=state.scale;};
 ...
 root.addEventListener('pointerdown',ev=>{if(ev.target.closest('button[data-zoom],button[data-action]'))return;pts.set(ev.pointerId,{x:ev.clientX,y:ev.clientY});if(pts.size===2){cancel();block=true;root.dataset.gesture='true';const [a,b]=[...pts.values()],r=root.getBoundingClientRect();gesture={distance:Math.hypot(a.x-b.x,a.y-b.y),scale:state.scale,mid:{x:(a.x+b.x)/2-r.left,y:(a.y+b.y)/2-r.top},x:state.x,y:state.y};ev.preventDefault();ev.stopPropagation();root.setPointerCapture(ev.pointerId);}else if(state.scale>1&&!ev.target.closest('[data-select],[data-handle],[data-vertex]')&&!['boundary','polygon','scale'].includes(root.querySelector('.plan-canvas')?.dataset.tool)){cancel();block=true;root.dataset.gesture='true';gesture={pan:true,start:{x:ev.clientX,y:ev.clientY},x:state.x,y:state.y};root.setPointerCapture(ev.pointerId);ev.preventDefault();ev.stopPropagation();}},true);
 ...
 apply();return {zoom,refresh:apply,reset:()=>{Object.assign(state,{scale:1,x:0,y:0});apply();},blocked:()=>block||performance.now()<Number(root.dataset.suppressUntil||0)};
}
